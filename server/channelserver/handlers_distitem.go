...
func handleMsgMhfGetDistDescription(s *Session, p mhfpacket.MHFPacket) {
	...
	row := s.server.db.QueryRowx("SELECT description FROM distribution WHERE id = $1", pkt.DistributionID)
	dist := &ItemDist{}
	err := row.StructScan(dist)
	if err != nil {
	...
	bf := byteframe.NewByteFrame()
	description, _ := stringsupport.ConvertUTF8ToShiftJIS(dist.Description)
